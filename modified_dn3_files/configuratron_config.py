...
class DatasetConfig:
    ...
    def _construct_session_from_config(self, session, sess_id, thinker_id):
        ...
        def load_and_prepare(sess):
            ...
            # use_annotations needs to be specified here as it depends on the events passed from config, 
            # not the ones obtained using a loader
            if 'events' in self._original_config:
                self.use_annotations = self.events is not None and True in [isinstance(x, str) for x in self.events.keys()]
            else:
                self.use_annotations = False
            
            # FIXME - make_epochs_from_raw gives an error if the events are not specified in the YAML config file
            # need to use a custom loader for each dataset that does not specify that
            if not 'events' in self._original_config:
                r, self.events = self._load_raw(sess)
            else:
        ...
        if self._create_raw_recordings:
            ...
            if not isinstance(raw, (mne.Epochs, mne.epochs.EpochsFIF)):  # Annoying other epochs type
                if self.use_annotations and self.annotation_format is not None:
                ...
                epochs = make_epochs_from_raw(raw, self.tmin, tlen, event_ids=self.events, baseline=self.baseline,
                                              ...
                                              use_annotations=self.use_annotations, chunk_duration=self.chunk_duration)
            else:
                epochs = raw

            # print(use_annotations)
            event_map = {v: v for v in self.events.values()} if self.use_annotations else self.events

            event_map = self.events if self.events is not None else {v: v for v in self.events.values()}
